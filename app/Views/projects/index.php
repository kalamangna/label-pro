...
<div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-gray-900 leading-none mb-1">Daftar Proyek</h1>
            <p class="text-sm text-gray-500 font-medium">
                Kelola proyek label Anda untuk pengelompokan yang lebih rapi
            </p>
        </div>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        <button data-modal-target="add-project-modal" data-modal-toggle="add-project-modal" class="flex-1 sm:flex-none inline-flex items-center justify-center px-6 py-3 border border-transparent rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100">
...
<div class="mb-4 p-6 bg-gray-50/50 border border-gray-200 rounded-2xl">
    <form action="/projects" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4">
        <!-- Search -->
        <div class="md:col-span-<?= session()->get('role') === 'admin' ? '3' : '6' ?> relative">
            <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                <i class="fa-solid fa-magnifying-glass text-slate-400 text-xs"></i>
            </div>
            <input type="text" name="search" value="<?= esc($search ?? '') ?>" class="block w-full ps-9 text-sm text-gray-900 border border-slate-300 rounded-xl bg-white focus:ring-slate-500 focus:border-slate-500 h-[42px] transition-all placeholder-slate-400" placeholder="Cari nama proyek...">
        </div>
        ...
        <div class="md:col-span-3 flex justify-end gap-2">
            ...
            <?php if (!empty($search) || ($sort ?? 'id') !== 'id' || !empty($userIdFilter)): ?>
                <a href="/projects" class="w-full sm:w-32 h-[42px] flex items-center justify-center bg-white border border-slate-300 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-50 transition-all active:scale-95 text-center">Reset</a>
            <?php endif; ?>
        </div>
    </form>
</div>
                ...
                <tr class="bg-white hover:bg-emerald-50/30 transition-colors">
                    ...
                    <?php if (session()->get('role') === 'admin'): ?>
                        <td class="px-6 py-4 text-center">
                            <span class="bg-gray-100 text-gray-800 text-xs font-bold px-2.5 py-0.5 rounded-full border border-gray-200">
                                <?= esc($project['added_by'] ?? 'Tidak diketahui') ?>
                            </span>
                        </td>
                    <?php endif; ?>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2 items-center">
                            <?php if (session()->get('role') !== 'admin'): ?>
                            <a href="/recipients/import?project_id=<?= $project['id'] ?>" class="p-2 text-gray-400 hover:text-emerald-600 transition-colors" title="Impor Data Excel">
                                <i class="fa-solid fa-file-import text-xs"></i>
                            </a>
                            <button type="button" class="p-2 text-gray-400 hover:text-emerald-600 transition-colors edit-project-btn"
                                    data-id="<?= $project['id'] ?>"
                                    data-name="<?= esc($project['name']) ?>"
                                    title="Edit">
                                <i class="fa-solid fa-pen-to-square text-xs"></i>
                            </button>
                            <button type="button" data-modal-target="delete-modal-<?= $project['id'] ?>" data-modal-toggle="delete-modal-<?= $project['id'] ?>" class="p-2 text-gray-400 hover:text-red-600 transition-colors" title="Hapus">
                                <i class="fa-solid fa-trash text-xs"></i>
                            </button>
                            <?php endif; ?>
                            <a href="/recipients?project_id=<?= $project['id'] ?>" class="text-[10px] font-bold text-emerald-600 hover:text-emerald-700 inline-flex items-center px-3 py-1.5 bg-emerald-50 rounded-lg ml-2 border border-emerald-100 transition-colors">
                                Lihat Data
                        ...
                        <!-- Delete Modal -->
                        <div id="delete-modal-<?= $project['id'] ?>" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full text-left">
                                <div class="relative bg-white rounded-2xl shadow-2xl border border-gray-100 text-left">
                                    <div class="p-6 text-center">
                                        ...
                                        <h3 class="mb-2 text-lg font-bold text-gray-900">Hapus proyek ini?</h3>
                                        <p class="mb-1 text-sm text-gray-500 italic">"<?= esc($project['name']) ?>"</p>
                                        <p class="mb-6 text-[10px] text-red-500 font-bold uppercase tracking-widest">Semua data penerima dalam proyek ini juga akan dihapus!</p>
                                        <div class="flex justify-center gap-3">
                                            <button data-modal-hide="delete-modal-<?= $project['id'] ?>" type="button" class="px-5 py-2.5 text-sm font-bold text-gray-500 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all">Batal</button>
                                            <a href="/projects/delete/<?= $project['id'] ?>" class="px-5 py-2.5 text-sm font-bold text-white bg-red-600 rounded-xl hover:bg-red-700 transition-all shadow-lg shadow-red-100">Hapus Proyek</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <tr>
                    <td colspan="<?= session()->get('role') === 'admin' ? '4' : '3' ?>" class="px-6 py-16 text-center bg-gray-50/30 text-gray-400 font-medium">
                        <div class="max-w-xs mx-auto text-center">
                            ...
                            <p class="text-sm font-bold text-gray-900 mb-1">Belum ada proyek</p>
                            <?php if (session()->get('role') === 'admin'): ?>
                                <p class="text-xs text-gray-500 leading-relaxed">Sistem belum memiliki proyek yang dibuat oleh pengguna.</p>
                            <?php else: ?>
                                <p class="text-xs text-gray-500 leading-relaxed">Mulai dengan membuat proyek pertama Anda untuk mengelompokkan penerima label.</p>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-2xl shadow-2xl border border-gray-100">
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                <h3 class="text-lg font-bold text-gray-900">Tambah Proyek Baru</h3>
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="add-project-modal">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <form action="/projects/store" method="POST" class="p-4 md:p-5 space-y-4 text-left">
                <?= csrf_field() ?>
                <div>
                    <label for="name" class="block mb-2 text-sm font-bold text-gray-900">Nama Proyek</label>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" data-modal-hide="add-project-modal" class="px-4 py-2 text-sm font-bold text-gray-500 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all">Batal</button>
                    <button type="submit" class="px-6 py-2 text-sm font-bold text-white bg-emerald-600 rounded-xl hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100">Simpan Proyek</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="edit-project-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-2xl shadow-2xl border border-gray-100">
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                <h3 class="text-lg font-bold text-gray-900">Edit Proyek</h3>
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-hide="edit-project-modal">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <form id="edit-project-form" action="" method="POST" class="p-4 md:p-5 space-y-4 text-left">
                <?= csrf_field() ?>
                <div>
                    <label for="edit-name" class="block mb-2 text-sm font-bold text-gray-900">Nama Proyek</label>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" data-modal-hide="edit-project-modal" class="px-4 py-2 text-sm font-bold text-gray-500 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all">Batal</button>
